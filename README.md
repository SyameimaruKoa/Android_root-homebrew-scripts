...
- スクリプト一覧
  ...
  - Wallpaper-PhotoEditor-sort.sh（壁紙素材の振り分け: 自分専用）
  - Wallpaper-twitter-sort.sh（Twitter画像整理: 自分専用）
- 注意事項
- 実装履歴
- 貢献・ライセンス
...
- 目的: Photo Editor 用フォルダへ移動して、壁紙素材の自動振り分けスクリプトを実行。
- 使い方:
  - `./Wallpaper-PhotoEditor-sort.sh`
- 実行内容: `bash ../Script/Wallpaper-AutoSort.sh .`
- 注意:
  - このスクリプトは汎用用途ではなく、作者の端末ディレクトリ構成向けの自分専用スクリプトです。
  - 呼び出している `Wallpaper-AutoSort.sh` は、`https://github.com/SyameimaruKoa/wallgen-env.git` 内のスクリプトを前提にしています。

### Wallpaper-twitter-sort.sh（Twitter画像整理: 自分専用）

- 目的: Twitter 用フォルダへ移動して、画像リネーム整理スクリプトを実行。
- 使い方:
  - `./Wallpaper-twitter-sort.sh`
- 実行内容: `bash ../../Script/rename-twitter.sh .`
- 注意:
  - このスクリプトは汎用用途ではなく、作者の端末ディレクトリ構成向けの自分専用スクリプトです。
  - 呼び出している `rename-twitter.sh` は、`https://github.com/SyameimaruKoa/wallgen-env.git` 内のスクリプトを前提にしています。
...
- Wallpaper 系スクリプトは特に作者個人のフォルダ構成に依存しており、汎用利用は想定していません。
...
- 2026-05-12: Wallpaper 系スクリプト2本の説明を追加し、「自分専用」表記と `wallgen-env` リポジトリ依存を明記。
